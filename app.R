...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    sliderInput(inputId="np", label="N for plot", min=0, max=1000, value=500,step=100)
...
server <- function(input, output) {
 ...
 #Between plot
  output$plot_b <- renderPlot({
    plot_b=plot(fsimdat_b(),pch=19, ylim=c(-3,3),xlim=c(-3,3),col=col_b, xlab="X between", ylab="Y between", main=paste("Between rho =", input$rho_b))
  })
 
  #Within plot 
  output$plot_w <- renderPlot({
    plot_w=plot(fsimdat_w(),pch=19, ylim=c(-3,3),xlim=c(-3,3), col=col_w, xlab="X within", ylab="Y within", main=paste("Within rho =", input$rho_w) )
  })
  
  #Cross-sectional plot
  output$plot_c <- renderPlot({
    plot_c=plot(fsimdat_c(),pch=19, ylim=c(-3,3),xlim=c(-3,3),col=col_c[fcol_c_select()], xlab="X cross-sectional", ylab="Y cross-sectional", main=paste("Cross-sectional rho =", round(frho_c(),2)) )
  })
